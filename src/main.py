...
def _web_set(key: str, value: str):
    try:
        import platform
...
def load_save():
    """Desktop: save.json | Web (emscripten): localStorage"""
    default = {"meta": 0, "upgrades": {k: 0 for k in UPGRADES}, "top_scores": [], "powers": {k: 0 for k in POWERS}}

    try:
        if _is_web():
            raw = _web_get(SAVE_KEY)
            if not raw:
                return default
            data = json.loads(raw)
        else:
...
def save_save(data):
    try:
        if _is_web():
            _web_set(SAVE_KEY, json.dumps(data, ensure_ascii=False))
        else:
...
def update_top_scores_scores" not in data or not isinstance(data["top_scores"], list):
            data["top_scores"] = []
        data["top_scores"] = [int(x) for x in data["top_scores"][:3]]
...
def update_top_scores(save_data, run_score):def update_top_scores(save_data, run_score):
...
async def menu_main(screen, font, big, save_data):
    ...
    while True:
        ...
        draw_text(screen, font, f"META: {save_data['meta']}   (save: {SAVE_FILE})", 80, 140, (200, 200, 200))
        # Top 3 skor kutusu
